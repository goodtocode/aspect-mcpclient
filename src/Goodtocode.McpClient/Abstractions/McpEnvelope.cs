...
namespace Goodtocode.Mcp
{
    public static class McpEnvelope
    {
        public static Envelope<T> Success<T>(string operation, T result, string? correlationId = null,
            ...
            => new Envelope<T>(operation, correlationId ?? McpCorrelationId.Get(), DateTimeOffset.UtcNow, result, null, cont, metadata);

        public static Envelope<T> Failure<T>(string operation, Problem problem, string? correlationId = null,
            IReadOnlyDictionary<string, string>? metadata = null)
            => new Envelope<T>(operation, correlationId ?? McpCorrelationId.Get(), DateTimeOffset.UtcNow, default, problem, null, metadata);
    }
